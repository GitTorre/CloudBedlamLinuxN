...
${TC} class add dev ${interface} parent 1:0 classid 1:1 cbq rate ${downrate}kbit allot 1500 prio 1 bounded isolated
${TC} class add dev ${interface} parent 1:0 classid 1:2 cbq rate ${uprate}kbit allot 1500 prio 1 bounded isolated
...
do
	...
	then
		...
		echo "IP address $address rate limited to $downrate kbs downstream, $uprate kbs upstream"
	else
		...
		echo "IPv6 address $address rate limited to $downrate kbs downstream, $uprate kbs upstream"
	fi
done
